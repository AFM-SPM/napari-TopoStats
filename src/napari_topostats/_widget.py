...
def start_loading_topostats():
    if os.environ.get("QT_QPA_PLATFORM") != "offscreen" and QApplication.instance() is not None:
        ...
        loading_spinner.start(message="Importing TopoStats")
...
MAIN_FUNCTIONS = [
    ...
    WidgetFunction(
        name="run_filters",
        function_key="filter",
        function_to_run=get_filter_image_func,
        type_class=get_filters_class,
    ...
    WidgetFunction(
        name="run_grains",
        function_key="grains",
        function_to_run=get_find_grains_func,
        type_class=get_grains_class,
        path_to_data="obj.topostats_object.full_mask_tensor[:, :, 1]",
        of_type=[Image],
