    ...
    if isinstance(return_value, np.ndarray):
        ...
        # If the return value is a greyscale image array, add it as an image layer
            ...
            viewer.add_image(
                ...
                name=name,
                contrast_limits=(-1, 5),
                metadata={"px2nm": original.metadata.get("px2nm", 1.0)}
                if original
                else {},
            )
        ...
        try:
            ...
            if self.type_class is not None:
                ...
                def func(**kwargs):
                    ...
                    if "image" in [
                        p.name
                        for p in including_config_params_from_function
                        + including_config_params_from_class
                    ]:
                    ...
                    if (
                        ...
                        in [
                            p.name
                            for p in including_config_params_from_class
                            + including_config_params_from_function
                        ]
                    ...
                    if method:
                        ...
                        if self.path_to_data.startswith("return"):
                            return_value = (
                                _eval(return_value, self.path_to_data[6:])
                                if len(self.path_to_data) > 6
                                else return_value
                            )
                        elif self.path_to_data.startswith("obj"):
                            return_value = (
                                _eval(instance, self.path_to_data[3:])
                                if len(self.path_to_data) > 3
                                else instance
                            )
                        else:
                            show_error_dialog(
                                f"Invalid path_to_data: {self.path_to_data}"
                            )
                ...
                def func(**kwargs):
                    ...
                    if return_value is not None:
                        ...
                        render_return_value(
                            ...
                            self.function_key,
                            viewer,
                            original=kwargs.get("image", None),
                            ndims=self.ndims,
                        )
                    else:
                        show_error_dialog(
